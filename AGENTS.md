...
- For any Tambo-owned URLs you include in comments or documentation, use the `tambo.co` domain (not `tambo.ai`). When you encounter existing `tambo.ai` URLs for Tambo-owned links, prefer updating them to `tambo.co` as part of related changes. External (non-Tambo) links are fine.
