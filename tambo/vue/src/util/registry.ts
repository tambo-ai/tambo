...
export const mapTamboToolToContextTool = (
  ...
  const parameters = getParametersFromZodFunction(tool.toolSchema as any);
  return {
    name: tool.name,
...
const getParametersFromZodFunction = (
  schema: z.ZodFunction<any, any> | ReturnType<typeof zodToJsonSchema>,
): ParameterSpec[] => {
  ...
  const parameters: z.ZodTuple = (schema as z.ZodFunction<any, any>).parameters();
