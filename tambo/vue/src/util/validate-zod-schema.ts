...
export function assertNoZodRecord(
  ...
  const visit = (current: ZodTypeAny, path: string[]): void => {
    ...
    if (typeName === ZodFirstPartyTypeKind.ZodRecord) {
      ...
      throw new Error(
        `z.record() is not supported in ${contextName}. Found at path "${joined}". Replace it with z.object({ ... }) using explicit keys.`,
      );
    }
    ...
    if (current instanceof z.ZodTuple) {
      (current as any).items.forEach((item: ZodTypeAny, idx: number) => visit(item, [...path, `${idx}`]));
      return;
    }
