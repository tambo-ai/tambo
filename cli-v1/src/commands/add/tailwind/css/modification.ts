...
export function addVariables(
  ...
  // Find existing TOP-LEVEL rule with the selector (not nested inside @media etc)
  ...
  if (!targetRule) {
    ...
    // Find the best place to insert it - after @import statements and @theme blocks
    // but before other content. Only consider direct children of root.
    let insertAfter: postcss.ChildNode | null = null;
    ...
    if (insertAfter) {
      ...
      if (lastImport) {
        root.insertAfter(lastImport, targetRule);
      } else {
...
export function mergeTheme(root: Root, newThemeVars: Map<string, string>) {
  ...
  if (!themeRule) {
    ...
    // Insert after @import statements, not at the beginning
    ...
    if (insertAfter) {
      root.insertAfter(insertAfter, themeRule);
    } else {
