...
import { updateImportPaths } from "../migrate-core.js";
...
export async function installComponents(
  ...
  const isExplicitPrefix = !!options.isExplicitPrefix;
...
export function cn(...inputs: ClassValue[]) {
  ...
  if (prodDeps.length > 0 || devDeps.length > 0) {
    ...
    try {
      const allowNonInteractive = !!options.yes;
  ...
  // Install the tambo skill for AI agent documentation
  if (!options.silent && !options.skipAgentDocs) {
    await installSkill({ prefix: options.installPath, silent: options.silent });
  }
}
