...
First, install the MCP client utilities:

```bash
npm install @modelcontextprotocol/sdk
```

Then configure the MCP client in your application:
...
**Note:** Client-side connections require the MCP server to support browser-based connections (typically via Server-Sent Events).
...
**Client-side:** Pass headers when creating the transport:
...
- For client-side connections, ensure the server supports SSE transport
