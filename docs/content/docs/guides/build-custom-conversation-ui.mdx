...
Messages contain text content, images, and generated components. The `renderedComponent` property contains any component Tambo created in response to the message. See the [Conversation Storage](/concepts/conversation-storage) concept for details on message structure.
...
You can also check the specific `generationStage` for more control over status display. See the [Conversation Storage](/concepts/conversation-storage) concept for available stages.
...
The `threads` array contains all stored conversations. See the [Conversation Storage](/concepts/conversation-storage) concept for details on thread structure.
