...
description: Build custom user interfaces for MCP prompts, resources, and elicitations
---
...
function renderField(
  ...
  if (fieldSchema.enum) {
    return (
      <select value={String(value)} onChange={(e) => handleChange(e.target.value)}>
...
```tsx
import { useState } from "react";
import { useTamboMcpPromptList, useTamboMcpResourceList } from "@tambo-ai/react/mcp";
