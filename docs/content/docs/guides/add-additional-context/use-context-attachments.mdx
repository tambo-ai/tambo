...
description: Stage temporary context that's automatically included in the next message
---

import LearnMore from "@/components/learn-more";
import { BookOpen } from "lucide-react";

This guide shows you how to use context attachments to stage temporary context that users explicitly add. Context attachments are perfect for including file contents, selected text, or other temporary context that should be cleared after the message is sent.

<LearnMore
  title="Understanding Context Attachments"
  description="Learn about the concept and when to use context attachments"
  href="/concepts/additional-context/context-attachments"
  icon={BookOpen}
/>
...
function StagedContextDisplay() {
  const { attachments, removeContextAttachment } =
    useTamboContextAttachment();
...
<button onClick={clearContextAttachments}>
  Clear All Staged Context
</button>;
```
...
<LearnMore
  title="Understanding Context Layers"
  description="Learn how context attachments combine with other context methods"
