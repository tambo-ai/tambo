...
| `TRUST_PROXY`         | No       | Trusted reverse-proxy hop count (`false` by default)                                     |
...
These variables control per-minute request limits. REST and MCP limits are
keyed by source address and endpoint because the global limiter runs before API
authentication. Counters are local to each API process unless shared storage is
configured.

| Variable               | Default | Description                           |
| ---------------------- | ------- | ------------------------------------- |
| `RATE_LIMIT_DEFAULT`   | `100`   | All NestJS API endpoints              |
| `RATE_LIMIT_STREAMING` | `20`    | LLM run and `advancestream` endpoints |
