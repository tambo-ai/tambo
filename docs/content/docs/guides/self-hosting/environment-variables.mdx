...
| `TRUST_PROXY`         | No       | Trusted reverse-proxy hop count (`false` by default; numeric only)                       |
...
| `RATE_LIMIT_STREAMING` | `100`   | LLM run and `advancestream` endpoints |
...
it to `false` when clients connect directly to the API.
