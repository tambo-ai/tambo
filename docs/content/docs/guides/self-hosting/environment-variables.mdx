...
validated project API key or source address and endpoint. MCP limits are keyed
by source address. Counters are local to each API process unless shared storage
is configured.
