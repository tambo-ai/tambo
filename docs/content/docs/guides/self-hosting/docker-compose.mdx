...
> **Note:** `NEXT_PUBLIC_TAMBO_API_URL` must be reachable from the browser. `http://localhost:8261` is fine for local development, but in production you should set it to your public API URL (for example `https://api.your-domain.com`), not `localhost` or an internal Docker hostname.
