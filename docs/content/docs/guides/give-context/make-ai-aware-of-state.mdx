...
Use context helpers to automatically include information about the user's current state on every message—no manual input required.
...
- ❌ Conversation-level instructions (use [Thread Instructions](/guides/give-context/shape-conversation-behavior))
...
- **Automatic**: Run on every message without explicit calls
- **Keyed**: Key name becomes the context name sent to AI
- **Flexible**: Can be sync or async functions
- **Optional**: Return `null` or `undefined` to skip including context
- **Clean**: No error propagation—handle errors internally
...
<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: "1rem", marginTop: "1.5rem" }}>
  ...
  />
  <LearnMore
    title="Shape Conversation Behavior"
    description="Set AI personality and rules with thread instructions"
    href="/guides/give-context/shape-conversation-behavior"
    icon={BookOpen}
  />
</div>
