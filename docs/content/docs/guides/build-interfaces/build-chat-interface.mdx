...
- Have registered [components](/guides/enable-generative-ui/register-components) with Tambo
        ...
        <div key={message.id} className={`message message-${message.role}`}>
          ...
          {message.role === "assistant" && message.toolCallRequest && (
            <div className="tool-calls">
              {message.toolCallRequest.tool_calls.map((toolCall) => (
                <div key={toolCall.id} className="tool-call">
                  <strong>Calling tool:</strong> {toolCall.function.name}
                </div>
              ))}
            </div>
          )}
