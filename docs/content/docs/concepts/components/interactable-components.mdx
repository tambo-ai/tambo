    ...
    <section>
      ...
      <div>
        ...
        <select
          id="note-color"
          value={currentColor}
          onChange={(event) => setCurrentColor(event.target.value as NoteProps["color"])}
        >
...
function Counter({ count, label }: CounterProps) {
  ...
  return (
    <section>
      ...
      <div>
        <button type="button" onClick={() => setCurrentCount((value) => value + 1)}>
          Increment
        </button>
        <button type="button" onClick={() => setCurrentCount((value) => value - 1)}>
          Decrement
        </button>
        <button type="button" onClick={() => setCurrentLabel("Items")}>Reset Label</button>
      </div>
    </section>
  );
}
...
Both components track user-editable state and rely on `useEffect` hooks to synchronize their local state whenever Tambo updates the props. Now Tambo is able to read and update these components in-place when responding to messages. Users can ask things like:
