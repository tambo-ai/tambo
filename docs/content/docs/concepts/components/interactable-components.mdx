...
When building child components inside an interactable (like inline editors or custom controls), use `useTamboInteractableComponent` to access the parent component's metadata.

### useTamboInteractableComponent

Returns the current interactable component context or `null` if used outside an interactable component.

<Callout type="warn" title="Only Works Inside Interactables">

This hook **ONLY** works when used inside a component that has been wrapped with `withInteractable`. It will return `null` if used anywhere else in your component tree.

</Callout>

```tsx
import { useTamboInteractableComponent } from "@tambo-ai/react";
...
function InlineEditor() {
  ...
  if (!component) {
    return null; // Not inside an interactable component
  }

  return (
    <div>
      <p>Editing: {component.name}</p>
...
- `id` - Unique identifier for this interactable component instance
- `componentName` - The component name (from `InteractableConfig.componentName`)
- `description` - The component description (from `InteractableConfig.description`)

**How it works**: When you wrap a component with `withInteractable`, it automatically provides a `TamboInteractableComponentProvider` that makes this context available to all child components.

### Common Use Cases

**Inline Editors**: Display which component is being edited

```tsx
// Child component using the hook
function ComponentEditor() {
  const component = useTamboInteractableComponent();

  return (
    <div className="editor">
      <h3>Editing {component?.name}</h3>
      {/* Editor controls here */}
    </div>
  );
}

// Parent interactable component
const InteractableNote = withInteractable(Note, {
  componentName: "Note",
...
function Note({ title, content }) {
  return (
    <div>
      <h3>{title}</h3>
      <p>{content}</p>
      <ComponentEditor /> {/* ✓ Hook works here */}
    </div>
  );
}
```

**Context-Aware Actions**: Pass component ID to update APIs
...
// Use inside an interactable:
function Note({ title, content }) {
  return (
    <div>
      <h3>{title}</h3>
      <p>{content}</p>
      <QuickActions /> {/* ✓ Can access component.id */}
    </div>
  );
}
```

**Conditional Rendering**: Show different UI based on component type
...
function ComponentBadge() {
  ...
  const badgeColor = component.name === "Note" ? "blue" : "gray";

  return <Badge color={badgeColor}>{component.name}</Badge>;
}

// Must be rendered inside an interactable component tree
```
