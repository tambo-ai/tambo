...
// Your existing presentational component
function Note({ title, content, color = "white" }: NoteProps) {
  return (
...
// Another simple presentational component
...
// Controlled wrappers to achieve two-way synchronization
function ControlledInteractableNote(props: NoteProps) {
  // Local state is the UI source of truth for user edits
  const [note, setNote] = useState<NoteProps>(props);
  ...
  // Sync down: when Tambo updates the interactable's props, reflect them in local state
  useEffect(() => {
    if (!interactableId) return;
    const match = snapshot.find((c) => c.id === interactableId);
    if (match) {
      // Replace with the latest props coming from Tambo
      setNote((prev) => ({ ...prev, ...(match.props as Partial<NoteProps>) }));
    }
  }, [snapshot, interactableId]);

  // User controls update local state, which updates props passed to InteractableNote.
  // The HOC then updates Tambo's registry (sync up).
  return (
    <section className={`note note-${note.color ?? "white"}`}>
      <InteractableNote
        {...note}
        onInteractableReady={(id) => setInteractableId(id)}
      />
      <div>
        ...
        <select
          ...
          onChange={(e) => {
            const color = e.currentTarget.value as NoteProps["color"];
...
function ControlledInteractableCounter(props: CounterProps) {
  const [counter, setCounter] = useState<CounterProps>(props);
  const [interactableId, setInteractableId] = useState<string | null>(null);
  const snapshot = useCurrentInteractablesSnapshot();

  // Sync down from Tambo updates
  useEffect(() => {
    if (!interactableId) return;
    const match = snapshot.find((c) => c.id === interactableId);
    if (match) {
      setCounter((prev) => ({
        ...prev,
        ...(match.props as Partial<CounterProps>),
      }));
    }
  }, [snapshot, interactableId]);
      ...
      <div>
        <button
          ...
          onClick={() => {
            ...
            setCounter((prev) => ({ ...prev, count: next }));
          }}
        >
          Increment
        </button>
        ...
        <button
          type="button"
          onClick={() => setCounter((prev) => ({ ...prev, label: "Items" }))}
        >
          Reset Label
        </button>
      </div>
...
function Page() {
  return (
    <main>
      <ControlledInteractableNote
        title="Welcome"
        content="This is a simple note that Tambo can update!"
        color="yellow"
      />
      <ControlledInteractableCounter count={42} label="Items" />
    </main>
  );
}
```

The controlled wrappers ensure two-way synchronization:

- User actions update local state, which updates the props passed into the interactable wrappers (sync up to Tambo's registry).
- When Tambo updates the interactable props, the wrappers detect those changes via `useCurrentInteractablesSnapshot()` and update local state (sync down to the UI).
