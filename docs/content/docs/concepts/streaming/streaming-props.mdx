...
component state. In those advanced cases, the `useTamboStreamingProps` hook
can help, but for typical streaming components you should prefer the
**stream → state → UI** pattern described in
[Building Streaming Components](/concepts/streaming/building-streaming-components).

<Callout type="warning" title="Deprecated for most components">
  `useTamboStreamingProps` is a low-level helper and is now deprecated for
  typical streaming components. For new work, start with the **stream → state →
  UI** pattern using
  ...
  exists as a reference for advanced cases where you truly need manual control
  over how streamed props are merged into state.
</Callout>
