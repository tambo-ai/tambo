...
<Callout type="warning" title="Low-level, deprecated helper">
  `useTamboStreamingProps` is a low-level helper and is now deprecated for most
  components. Prefer the **stream → state → UI** pattern described in [Building
  Streaming Components](/concepts/streaming/building-streaming-components). The
  **[Streaming Props into State](/concepts/streaming/streaming-props)** page
  covers this helper in more detail for advanced use cases.
</Callout>
