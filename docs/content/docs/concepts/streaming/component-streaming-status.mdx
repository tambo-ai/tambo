...
For a full example that combines streaming status with editable component
state, see
...
### With Streaming Props Hook (advanced)
...
<Callout type="warning" title="Low-level, deprecated helper">
  `useTamboStreamingProps` is a low-level helper and is **deprecated**. Prefer
  the **stream → state → UI** pattern described in [Building Streaming
  Components](/concepts/streaming/building-streaming-components) for new and
  existing components. The **[Streaming Props into
  State](/concepts/streaming/streaming-props)** page documents the legacy
  behavior for apps that still rely on this helper while they migrate.
</Callout>
