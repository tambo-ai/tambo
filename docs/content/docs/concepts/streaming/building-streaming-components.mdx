...
The older `useTamboStreamingProps` hook is now **deprecated**. It exists only
for backwards compatibility with apps that already depend on it.

Avoid this hook in new components. If you absolutely cannot use
`useTamboComponentState` (for example, because you're wiring into a pre-existing
state store) and already rely on `useTamboStreamingProps`, keep that usage
isolated and plan to migrate when possible rather than expanding it further.
