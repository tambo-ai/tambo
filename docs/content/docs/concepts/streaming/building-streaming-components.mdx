...
avoid React controlled/uncontrolled warnings. In older versions of this
example you may see `draft.subject ?? ""` or similar fallbacks; those are
still appropriate when your state type allows `null`/`undefined`. In this
updated version, `EmailComposerState` is non-nullable, so the JSX no longer
needs the `?? ""` pattern.
