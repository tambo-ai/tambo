...
Type "/" at the start of an empty text field to quickly access prompts:
...
Alternatively, click the prompts button to browse available templates:
...
If you don't specify `serverKey`, Tambo derives one from the URL (e.g., `https://mcp.linear.app/mcp` becomes `linear`). For more details, see [Server namespacing](/concepts/model-context-protocol/providers/clientside-mcp-connection#server-namespacing).
