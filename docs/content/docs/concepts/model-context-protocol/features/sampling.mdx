...
**Important:** Sampling currently only works with **[server-side MCP connections](/concepts/model-context-protocol/serverside-mcp-connection)**. Client-side MCP servers cannot make sampling requests at this time.
...
sequenceDiagram
    ...
    Tambo->>MCP Server: Call weather tool
    MCP Server->>MCP Server: Fetch raw weather data
    MCP Server->>Tambo: Sampling request: "Summarize this weather data"
    Tambo->>LLM: Generate summary
    LLM->>Tambo: Summary response
    Tambo->>MCP Server: Sampling result
    MCP Server->>Tambo: Tool result with summary
