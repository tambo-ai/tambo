...
export default function RootLayout({
  ...
  return (
    <TamboProvider 
      ...
      voiceInputEnabled={true} // Enable voice input (default: true)
    >
      {children}
    </TamboProvider>
  );
...
The `TamboProvider` supports voice input functionality out of the box. Users can click the microphone button in the message input to record their message instead of typing. The audio is transcribed using OpenAI's Whisper model through your backend.

To disable voice input:
...
<Callout type="info" title="Voice Input Requirements">
