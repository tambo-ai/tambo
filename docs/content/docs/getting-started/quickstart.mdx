...
This walks you through creating a project, generates an API key, and adds it to `.env.local` by default.

If `.env.local` doesn't exist but `.env` does, it'll write the key to `.env` instead.
