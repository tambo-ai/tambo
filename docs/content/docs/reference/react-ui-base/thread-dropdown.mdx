...
export function DemoThreadDropdown() {
  return (
    <ThreadDropdown.Root>
      <ThreadDropdown.Trigger className="rounded border px-3 py-1">
        Threads
      </ThreadDropdown.Trigger>
      <ThreadDropdown.Content
        render={(props, state) => (
          <div {...props} className="rounded border bg-white p-2 shadow">
            <ThreadDropdown.NewThread className="w-full rounded px-2 py-1 hover:bg-gray-100">
              New Thread
            ...
            {state.threads.map((thread) => (
              <ThreadDropdown.ThreadItem
                key={thread.id}
                thread={thread}
                className="w-full rounded px-2 py-1 hover:bg-gray-100"
              >
                {thread.id}
              </ThreadDropdown.ThreadItem>
            ))}
...
<ThreadDropdown.NewThread ref={newThreadRef}>New Thread</ThreadDropdown.NewThread>
```
