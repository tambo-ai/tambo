...
export function DemoMcpPrompts() {
  return (
    <McpPrompts.Root onInsertText={(text) => console.log("Insert:", text)}>
      <McpPrompts.Trigger className="rounded bg-blue-500 px-3 py-1 text-white">
        Insert Prompt
      </McpPrompts.Trigger>
      <McpPrompts.List
        render={(props, state) => (
          <div {...props} className="flex flex-col gap-1">
            {state.prompts.map((entry) => (
              <McpPrompts.Item
                ...
                className="rounded px-2 py-1 hover:bg-gray-100 data-selected:bg-blue-100"
              >
                {entry.prompt.name}
              </McpPrompts.Item>
            ))}
      ...
      <McpPrompts.Error
        render={(props, state) => (
          <div {...props} className="text-red-500 text-sm">
            {state.error}
          </div>
        )}
      />
...
The `Error` part only renders when the selection lifecycle enters the `error` state (invalid format, no text content, or fetch failure). Errors auto-clear after 3 seconds.
...
| -------------- | --------------------------- | ---------------------------------------------------- |
| `onInsertText` | `(text: string) => void`    | Callback invoked when prompt text is ready.          |
