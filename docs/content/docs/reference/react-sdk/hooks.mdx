...
<Callout type="info" title="Auth-gating">
  This hook automatically disables the query until the user is authenticated
  (`authState.status === "identified"`). This prevents 401 errors during token
  exchange. You can pass additional `enabled` logic via the `options` parameter
...
<Callout type="info" title="Auth-gating">
  ...
  user is authenticated. You can pass additional `enabled` logic via
  `queryOptions` — both conditions must be true for the query to run.
</Callout>
