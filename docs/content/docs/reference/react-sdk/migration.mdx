...
description: Step-by-step guide for migrating from @tambo-ai/react/legacy to @tambo-ai/react.
---

This guide walks you through migrating from the legacy React SDK (`@tambo-ai/react/legacy`) to the current SDK (`@tambo-ai/react`). Follow the steps in order — each one builds on the previous.

## Overview of changes

The v1 SDK is a redesign focused on explicit control and composability. The main shifts are:
...
Change your import path from `@tambo-ai/react/legacy` to `@tambo-ai/react`:
...
```tsx title="After"
import { V1ComponentRenderer, type TamboV1Message } from "@tambo-ai/react";
...
The interactable HOC and hooks are available with updated names:
...
The `TamboContextAttachmentProvider` is included in the V1 provider hierarchy, and the `useTamboContextAttachment` hook is available from the main package:
...
| Legacy (`@tambo-ai/react/legacy`) | V1 preview (`@tambo-ai/react`)        | V1 stable (future `@tambo-ai/react`)  |
| --------------------------------- | ------------------------------------- | ------------------------------------- |
