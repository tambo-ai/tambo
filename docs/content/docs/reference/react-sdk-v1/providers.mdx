...
| `userToken`              | `string`                                         | Yes\*    | OAuth token for user authentication    |
...
<Callout type="info" title="userKey vs contextKey">
  In V1, threads are scoped per user using `userKey` (or the user identified by
  `userToken`). This replaces `contextKey` from the current SDK. The `userKey`
  is a unique identifier for the user, such as a user ID from your
