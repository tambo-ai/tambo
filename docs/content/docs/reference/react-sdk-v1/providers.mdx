...
<Callout type="info" title="userKey vs contextKey">
  In V1, threads are scoped per user using `userKey`. This replaces `contextKey` from the current SDK. The `userKey` is a unique identifier for the user, such as a user ID from your authentication system.
</Callout>
