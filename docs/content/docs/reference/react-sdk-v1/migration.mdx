...
Usage is unchanged — `withTamboInteractable` wraps a component the same way:
...
The `TamboContextAttachmentProvider` is included in the V1 provider hierarchy. The `useTamboContextAttachment` hook is available from the base SDK import path:
...
| `withTamboInteractable`             | `withTamboInteractable`               | `withTamboInteractable`               |
