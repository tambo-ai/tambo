...
| `propStatus`   | `Partial<Record<keyof Props, PropStatus>>`                   | Per-prop streaming status. Keys are absent until the prop receives its first token. See [`PropStatus`](/reference/react-sdk-v1/types#propstatus). |
