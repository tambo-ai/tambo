...
interface TamboTool {
  ...
  description: string;
  tool: (...args: any[]) => any;
  inputSchema?: z.ZodTypeAny | JSONSchemaLite;
  outputSchema?: z.ZodTypeAny | JSONSchemaLite;
  transformToContent?: (
    result: any,
...
A Zod schema that defines the tool's input parameters. Can also be a JSON Schema object. Omit if the tool takes no parameters.
...
```typescript
inputSchema: z.string().describe("The city name");
```
