...
Detailed guidance for Claude Code agents working inside `apps/dashboard`, the Next.js dashboard for Tambo Cloud.
...
- **Styling & Fonts**: Tailwind configured in `tailwind.config.ts`; fonts wired via `apps/dashboard/lib/fonts.ts`.
...
Use this guide every time you touch `apps/dashboard` so the dashboard stays consistent with the rest of the monorepo.
