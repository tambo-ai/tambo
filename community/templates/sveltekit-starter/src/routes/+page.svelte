...
<div class="h-screen w-full flex flex-col">
	...
	<header class="flex items-center justify-between px-4 py-2 border-b border-border bg-background">
		<div class="flex items-center gap-2">
			...
			<span class="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded">SvelteKit</span>
		</div>
		<button
			type="button"
		...
		<div class="flex-1 flex items-center justify-center p-4">
			<div class="max-w-md text-center space-y-4">
				...
				<p class="text-muted-foreground">
					To get started, create a <code class="bg-muted px-1 rounded">.env</code> file in the project root with your Tambo API key:
				</p>
				<pre class="bg-muted p-4 rounded-lg text-sm text-left"><code>VITE_TAMBO_API_KEY=your_api_key_here</code></pre>
				<p class="text-sm text-muted-foreground">
					Get your API key at <a href="https://tambo.co" class="text-blue-600 hover:underline" target="_blank" rel="noopener noreferrer">tambo.co</a>
				</p>
			</div>
		</div>
	{:else}
