...
const NodeIcon = ({ label, type }: { label: string; type: string }) => {
  ...
  if (l.includes("github") || l.includes("trigger"))
    return <Webhook className="w-5 h-5 text-indigo-500" />;
  if (l.includes("slack") || l.includes("message"))
    return <MessageSquare className="w-5 h-5 text-pink-500" />;
  if (l.includes("summary") || l.includes("llm"))
  ...
  return (
    ...
    >
      ...
      <Handle
        ...
        className="!bg-slate-300 !w-3 !h-3"
      />
