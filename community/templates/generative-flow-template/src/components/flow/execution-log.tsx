...
export function ExecutionLog({ className, executionResults, isRunning, ...props }: ExecutionLogProps) {
    ...
    return (
        <div className={cn("flex flex-col h-full bg-black/90 text-green-400 font-mono text-sm p-4 border-t border-slate-800", className)} {...props}>
            <div className="flex justify-between items-center mb-2 border-b border-slate-700 pb-2">
                ...
                <span className={cn("w-2 h-2 rounded-full", isRunning ? "bg-green-500 animate-pulse" : "bg-slate-500")} />
            </div>
