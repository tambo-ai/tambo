      ...
      {tasks.map((task) => (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span
              className={`text-sm font-medium leading-none ${task.completed ? "text-muted-foreground line-through" : "text-foreground"}`}
            >
              {task.title}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <span
              className={`text-[10px] px-2 py-0.5 rounded-full font-bold tracking-tighter uppercase ${
                task.completed
