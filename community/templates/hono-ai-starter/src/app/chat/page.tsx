    ...
    <div className="flex flex-col h-full bg-background text-foreground transition-colors duration-300">
      <header className="p-4 border-b border-border bg-card/50 backdrop-blur-md flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xs font-bold tracking-widest uppercase text-muted-foreground">Hono x Tambo</h1>
        </div>
        <div className="flex gap-2">
           <span className="text-[10px] bg-secondary px-2 py-0.5 rounded border border-border font-mono">EDGE_V1</span>
        </div>
      </header>
        ...
        {thread.messages
          ...
          >
            <div className={`max-w-[90%] md:max-w-[75%] space-y-2`}>
              <div className={`rounded-2xl p-4 text-sm border shadow-sm ${
                message.role === 'user' 
              ...
              <div className={`flex items-center gap-2 px-1 text-[10px] uppercase tracking-tighter text-muted-foreground font-medium ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {message.role}
              </div>
            </div>
          </div>
