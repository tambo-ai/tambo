      ...
      <div className="chat-scroll flex-1 overflow-y-auto p-5">
        {!thread?.messages?.length ? (
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <h3 className="mb-2 font-semibold text-slate-900">Start a conversation</h3>
                  ...
                  >
                    ...
                    >
                      ...
                      {message.renderedComponent && (
                        <div className="mt-3 rounded-xl bg-white p-1">{message.renderedComponent}</div>
                      )}
                    </div>
                  </div>
                );
    ...
    for (let i = 0; i < content.length; i++) {
      ...
      if (typeof part === "string") {
        ...
        if (part.startsWith("{") && part.includes('"message"')) {
          try {
            ...
            if (parsed.message) {
              renderedParts.push(<p key={i} className="whitespace-pre-wrap">{parsed.message}</p>);
              continue;
            }
          } catch {
            // Not valid JSON
        ...
        renderedParts.push(<p key={i} className="whitespace-pre-wrap">{part}</p>);
        continue;
      }
      ...
      if (typeof part === "object" && part !== null) {
        ...
        if ("message" in part) {
          ...
          renderedParts.push(<p key={i} className="whitespace-pre-wrap">{obj.message}</p>);
          continue;
        }
        
        // Content part with type
        if ("type" in part) {
          const typedPart = part as { type: string; text?: string; result?: unknown; content?: unknown };
          
          if (typedPart.type === "text" && typedPart.text) {
            renderedParts.push(<p key={i} className="whitespace-pre-wrap">{typedPart.text}</p>);
            continue;
          }
          
          // Handle tool results - extract just the message
          if (typedPart.type === "tool-result") {
            const result = (typedPart.result ?? typedPart.content) as { message?: string } | undefined;
            if (result?.message) {
              renderedParts.push(<p key={i} className="whitespace-pre-wrap">{result.message}</p>);
            }
