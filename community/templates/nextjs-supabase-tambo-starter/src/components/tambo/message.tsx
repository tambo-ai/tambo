...
const messageVariants = cva("flex w-full", {
  variants: {
    variant: {
      ...
      solid: [
        // Message container
        "[&>div>div:first-child]:bg-black/70",
        "[&>div>div:first-child]:border",
        "[&>div>div:first-child]:border-neutral-800",

        // Typography-first, not card-first
        "[&>div>div:first-child]:text-neutral-100",

        // No glow, no blur
        "[&>div>div:first-child]:shadow-none",

        // Minimal interaction feedback
        "[&>div>div:first-child]:transition-colors",
        "[&>div>div:first-child]:hover:bg-neutral-800",
      ].join(" "),
    },
  },
  defaultVariants: {
...
const LoadingIndicator: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  ...
  return (
    <div className={cn("flex items-center gap-1 text-neutral-400", className)} {...props}>
      ...
      >
        <div className="flex flex-col w-full">
          <button
            ...
            className={cn(
              "flex items-center gap-1 cursor-pointer rounded-md p-1 select-none w-fit hover:bg-purple-500/10 hover:text-purple-300 transition-colors",
            )}
          >
            <ToolcallStatusIcon
