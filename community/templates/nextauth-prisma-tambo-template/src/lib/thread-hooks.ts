    ...
    () => (element: T) => {
      refs.forEach((ref) => {
        ...
        if (typeof ref === "function") {
          ...
          // Using Object.assign to avoid direct mutation that ESLint dislikes
...
export function usePositioning(
  ...
  const getHistoryPosition = (): "left" | "right" => {
    if (isRightClass) return "right";
    if (hasCanvasSpace && canvasIsOnLeft) return "right";
    return "left";
  };
