...
export function createTools(role: "admin" | "user") {
    return [
        defineTool({
            ...
            description: "Retrieves analytics and health metrics for the system. Requires Admin privileges.",
            inputSchema: z.object({
                verbose: z.boolean().optional().describe("If true, returns detailed sub-system breakdowns."),
            }),
            ...
            tool: async (params) => {
                ...
                if (role !== "admin") {
                    return {
                        status: "denied",
                        message: "Access Denied. Your current security clearance level is 'User'. Administrative privileges are required to access System Health metrics and infrastructure diagnostics."
                    };
                }
                ...
                return {
                    status: "optimal",
                    message: "System scan complete. All core infrastructure components are operating within optimal parameters. No performance bottlenecks or security anomalies detected.",
                    data: {
                        cpu: params.verbose ? "14.2% (Kernel: 2.1% | User: 12.1%)" : "14.2%",
        ...
        defineTool({
            ...
            tool: async () => ({
                status: "active",
                message: "Identity verified. Displaying current user profile and session data retrieved from the BetterAuth security layer."
            }),
        }),
    ];
}
