...
import { Bot, Mic, MicOff, Send, User as UserIcon, AlertCircle } from "lucide-react";
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        return () => window.removeEventListener('populateInput', handlePopulate as EventListener);
    }, []);
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (errorMsg.includes("streaming") || errorMsg.includes("response") || errorMsg.includes("network")) {
                setGlobalError("Network error detected. Please check your internet connection or API availability and try again.");
            } else {
        ...
        <div className="h-[100dvh] bg-zinc-950 text-zinc-100 flex flex-col font-[family-name:var(--font-geist-sans)] overflow-hidden dark">
            ...
            >
                {messages.length === 0 && !streaming && (
                    <div className="flex flex-col items-center justify-center min-h-[400px] text-center space-y-6 animate-in fade-in zoom-in duration-700">
                        ...
                        <div className="space-y-2">
                            <h2 className="text-3xl font-black text-white tracking-tight">How can I assist you?</h2>
                            <p className="text-zinc-400 text-sm max-w-xs mx-auto leading-relaxed">
                                Use voice or text to explore system diagnostics, user profiles, or general assistance.
                            </p>
                        </div>
                ...
                {(messages || []).map((message: TamboMessage, messageIdx: number) => (
                    ...
                    >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border shadow-lg ${message.role === "user"
                            ? "bg-zinc-800 border-zinc-700 text-zinc-400"
                            : "bg-indigo-600 border-indigo-500 text-white"
                            }`}>
                            {message.role === "user" ? <UserIcon size={14} /> : <Bot size={14} />}
                        </div>
                        <div className={`max-w-[85%] space-y-2 ${message.role === "user" ? "text-right" : "text-left"}`}>
                            <div className={`inline-block px-5 py-3.5 rounded-[22px] text-sm leading-relaxed font-sans shadow-xl ${message.role === "user"
                                ...
                                {(Array.isArray(message.content) ? message.content as TamboContentPart[] : []).map((part, i) => {
                                    if (part.type === "text") {
                                        // 🛡️ HEURISTIC: Catch JSON Hallucinations (Crucial for this template's stability)
                                        const text = part.text || "";
                                        if (text.includes('{') && text.includes('}') && (text.includes('"status"') || text.includes('"cpu"'))) {
                                            try {
                                                const jsonMatch = text.match(/\{[\s\S]*\}/);
                                                if (jsonMatch) {
                                                    const data = JSON.parse(jsonMatch[0]) as Record<string, unknown>;
                                                    const status = (data.status || "") as string;
                                                    const innerData = (data.data || data) as { cpu?: string; disk?: string; bandwidth?: string; uptime?: string };
                                                    const cpu = innerData.cpu;
                                                    const messageVal = (data.message || "") as string;

                                                    if (status === "optimal" || status === "warning" || status === "critical" || status === "denied" || cpu) {
                                                        return (
                                                            <div key={`heuristic-stats-${i}`} className="mt-4 pt-4 border-t border-white/5 w-full">
                                                                <AdminStats data={innerData} status={status as "optimal" | "warning" | "critical" | "denied"} message={messageVal} />
                                                            </div>
                                                        );
                                                    } else if (status === "active") {
                                                        return (
                                                            <div key={`heuristic-user-${i}`} className="mt-4 pt-4 border-t border-white/5 w-full">
                                                                <UserCard />
                                                            </div>
                                                        );
                                                    }
                                    ...
                                    if (part.type === "tool-result" || part.type === "tool_result") {
                                        ...
                                        const result = (typeof rawResult === 'object' && !Array.isArray(rawResult)) ? rawResult : null;
                                        const componentConfig = tamboComponents.find(c => c.name === toolName);

                                        if (componentConfig && result) {
                                            const Component = componentConfig.component;
                                            return (
                                                <div key={i} className="mt-4 pt-4 border-t border-white/10 w-full flex justify-center">
                                                    <Component {...result} />
                                                </div>
                                            );
                                        }
                                        ...
                                        if (rawResult) {
                                            return (
                                                <div key={i} className="mt-2 p-3 bg-white/5 rounded-xl border border-white/5 font-mono text-xs overflow-auto max-h-[200px]">
                                    ...
                                    if (part.type === "component" && part.name) {
                                        const componentConfig = tamboComponents.find(c => c.name === part.name);
                                        if (componentConfig) {
                                            const Component = componentConfig.component;
                                            return (
                                                <div key={i} className="mt-4 pt-4 border-t border-white/10 w-full flex justify-center">
                            ...
                            <div className="text-[10px] text-muted-foreground uppercase tracking-wider px-1" suppressHydrationWarning>
                ...
                <div className="max-w-3xl mx-auto">
                    ...
                    <div className="flex gap-2 items-center bg-white/5 border border-white/10 p-2 rounded-full focus-within:border-indigo-500/50 focus-within:ring-2 focus-within:ring-indigo-500/10 transition-all duration-200">
                        <button
                            ...
                            className={`p-2.5 rounded-full transition-all duration-200 ${isSupported
                                ? isListening
                                    ? "bg-red-500/20 text-red-500 animate-pulse ring-1 ring-red-500/50"
                                    : "text-muted-foreground hover:bg-white/10 hover:text-white"
                                : "opacity-30 cursor-not-allowed text-zinc-600"
                                }`}
                            title={!isSupported ? "Voice not supported in this browser" : isListening ? "Stop Listening" : "Start Voice"}
                        >
