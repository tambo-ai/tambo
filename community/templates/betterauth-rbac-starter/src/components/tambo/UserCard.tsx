...
export function UserCard() {
    ...
    const userJoined = session.user.createdAt ? new Date(session.user.createdAt).toLocaleDateString() : "Recently";
        ...
        <div className="group relative w-full max-w-[350px] mx-auto opacity-100 scale-100 transition-all duration-500">
            ...
            <div className="relative p-8 bg-black border border-white/10 rounded-[30px] overflow-hidden">
                <div className="flex items-center gap-6 mb-8">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border transition-all duration-300 ${userRole === "admin"
                ...
                <div className="space-y-3">
                    <InfoRow icon={<Mail size={14} />} label="Protocol" value={userEmail} />
                    <InfoRow icon={<Calendar size={14} />} label="Enrolled" value={userJoined} />
                    <InfoRow icon={<Settings size={14} />} label="Entity ID" value={userId.slice(0, 16)} />
                </div>
                ...
                <button className="w-full mt-8 py-3 bg-zinc-900/50 hover:bg-zinc-900 border border-white/5 hover:border-white/10 rounded-2xl text-xs font-black uppercase tracking-[0.2em] text-zinc-400 hover:text-white transition-all flex items-center justify-center gap-2 group/btn">
                    Access Neural Settings <ChevronRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
                </button>
            </div>
        </div>
    );
...
function InfoRow({ icon, label, value }: { icon: React.ReactNode, label: string, value: string }) {
    return (
        <div className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl group/row hover:border-white/10 transition-colors">
            ...
            <span className="text-sm font-bold text-zinc-300 group-hover/row:text-white transition-colors truncate max-w-[150px]">{value}</span>
        </div>
    );
}
