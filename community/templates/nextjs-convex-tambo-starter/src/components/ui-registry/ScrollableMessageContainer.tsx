...
>(({ className, children, ...props }, ref) => {
  ...
  // Reset autoscroll when new messages are added
  useEffect(() => {
    if (
      messagesContent &&
      messagesContent.length > prevMessageCountRef.current
    ) {
      setShouldAutoscroll(true);
      prevMessageCountRef.current = messagesContent.length;
    }
  }, [messagesContent]);
  ...
  useEffect(() => {
    if (scrollContainerRef.current && messagesContent && shouldAutoscroll) {
      ...
      if (generationStage === GenerationStage.STREAMING_RESPONSE) {
        // During streaming, scroll immediately using animation frame
        // This is more reliable than setTimeout for high-frequency updates
