...
import { MessageGenerationStage } from "./MessageGenerationStage";
import { Tooltip, TooltipProvider } from "./SuggestionsTooltip";
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "flex gap-2 justify-between w-full pb-2 rounded-md bg-transparent min-h-[2.5rem]",
      ...
      {suggestions.length > 0
        ? suggestions.map((suggestion, index) => (
            ...
            >
              <button
                className={cn(
                  "flex-1 py-2 px-2.5 rounded-2xl text-xs transition-colors truncate",
                  "border border-flat",
        ...
        : // Render placeholder buttons when no suggestions are available
          placeholders.map((_, index) => (
            <div
              ...
              className="flex-1 py-2 px-2.5 rounded-2xl text-xs border border-flat bg-muted/20 text-transparent animate-pulse"
