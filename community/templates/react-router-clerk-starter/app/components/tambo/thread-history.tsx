...
export const ThreadHistory = React.forwardRef<HTMLDivElement, ThreadHistoryProps>(
    (
        {
            className,
            contextKey,
 ...
 */
export const ThreadHistoryNewButton = React.forwardRef<
    HTMLButtonElement,
...
>(({ className, ...props }, ref) => {
    ...
    const filteredThreads = useMemo(() => {
        ...
        return threads.items.filter((thread: TamboThread) => {
            // Filter by contextKey if present
            // Cast to unknown first to avoid TS intersection errors with library types
            const t = thread as unknown as { contextKey?: string };
            if (contextKey && t.contextKey !== contextKey) {
                return false;
            }
    ...
    }, [isCollapsed, threads, searchQuery, contextKey]);
