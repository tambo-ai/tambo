...
jest.unstable_mockModule("../../src/commands/shared/component-utils.js", () => {
  ...
  const detectCrossLocationDependencies = async (
    ...
    for (const component of verifiedComponents) {
      const configPath = `/mock-project/cli/src/registry/${component.name}/config.json`;
...
jest.unstable_mockModule("../../src/utils/dependency-resolution.js", () => {
  ...
  const expandComponentsWithDependencies = async (
    ...
    for (const dep of result.installedDependencies) {
      if (!existingNames.has(dep)) {
        ...
        const findLocation = (
          await import("../../src/commands/shared/component-utils.js")
        ).findComponentLocation;
