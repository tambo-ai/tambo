...
describe("streamReducer", () => {
  ...
  describe("tambo.tool_call.args_delta event", () => {
    it("applies JSON Patch operations to tool_use input", () => {
      const state = createTestStreamState("thread_1");
      state.threadMap.thread_1.thread.messages = [
        {
          id: "msg_1",
      ...
      const result = streamReducer(state, {
        ...
        event: {
          ...
          value: {
            toolCallId: "tc_1",
            operations: [{ op: "add", path: "/city", value: "Seattle" }],
            streamingStatus: { city: "started" },
          },
          timestamp: Date.now(),
        } as CustomEvent,
        threadId: "thread_1",
      ...
      const content = asToolUseContent(
        result.threadMap.thread_1.thread.messages[0].content,
        0,
      );
      expect(content.input).toEqual({ city: "Seattle" });
    });

    it("incrementally patches tool_use input across multiple events", () => {
      const state = createTestStreamState("thread_1");
      state.threadMap.thread_1.thread.messages = [
        {
          id: "msg_1",
      ...
      // First patch: add city
      const state2 = streamReducer(state, {
        type: "EVENT",
        event: {
          type: EventType.CUSTOM,
          name: "tambo.tool_call.args_delta",
          value: {
            toolCallId: "tc_1",
            operations: [{ op: "add", path: "/city", value: "Se" }],
            streamingStatus: { city: "started" },
          },
          timestamp: Date.now(),
        } as CustomEvent,
      ...
      // Second patch: replace city with updated value
      const state3 = streamReducer(state2, {
        type: "EVENT",
        event: {
          type: EventType.CUSTOM,
          name: "tambo.tool_call.args_delta",
          value: {
            toolCallId: "tc_1",
            operations: [{ op: "replace", path: "/city", value: "Seattle" }],
            streamingStatus: { city: "streaming" },
          },
          timestamp: Date.now(),
        } as CustomEvent,
        threadId: "thread_1",
      ...
      const content = asToolUseContent(
        state3.threadMap.thread_1.thread.messages[0].content,
        0,
      );
      expect(content.input).toEqual({ city: "Seattle" });
    });
  ...
  describe("tambo.tool_call.end event", () => {
    it("sets final args on tool_use content block", () => {
      const state = createTestStreamState("thread_1");
      state.threadMap.thread_1.thread.messages = [
        {
          id: "msg_1",
          ...
          content: [
            {
              ...
              id: "tc_1",
              name: "get_weather",
              input: { city: "Se" }, // partial from streaming
            },
          ],
          createdAt: "2024-01-01T00:00:00.000Z",
        },
