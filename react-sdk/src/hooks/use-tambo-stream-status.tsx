...
function usePropsStreamingStatus<Props extends Record<string, any>>(
  ...
  const [propTracking, setPropTracking] = useState<
    Record<
      string,
      { hasStarted: boolean; isComplete: boolean; error?: Error }
    >
  >({});
    ...
    Object.keys(props).forEach((key) => {
      ...
      result[key as keyof Props] = {
        ...
        isStreaming:
          tracking.hasStarted &&
          !tracking.isComplete &&
          (isGenerationStreaming || !tracking.isComplete),
...
function deriveGlobalStreamStatus<Props extends Record<string, any>>(
  ...
  return {
    ...
    isStreaming:
      isGenerationStreaming || propStatuses.some((p) => p.isStreaming),
...
>(): {
  ...
  const componentProps =
    (message?.component?.props as Props) || ({} as Props);
  ...
  const streamStatus = useMemo(() => {
    const generationError = message?.error
      ? new Error(message.error.message || "Generation error")
      : undefined;
