...
export const convertPropsToJsonSchema = (component: RegisteredComponent): any => {
  if (!component.props) {
...
export const getDefaultContextAdditions = (): string[] => {
  ...
  return [`The current time in user's timezone (${utcOffset}) is: ${new Date().toLocaleString()}`];
};
...
export const mapTamboToolToContextTool = (tool: TamboTool): ComponentContextToolMetadata => {
...
const getParametersFromZodFunction = (schema: z.ZodFunction<any, any>): ParameterSpec[] => {
