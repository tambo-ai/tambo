...
<task type="auto">
  ...
  <action>
    ...
    - async function executeRun(client: TamboClient, threadId: string, message: SendMessageParams, options?: RunOptions): Promise<Message>
      ...
      - Start streaming via client.stream() to POST /threads/{threadId}/runs
      ...
      - When all tool calls in a round complete:
        - Send tool results back: POST /threads/{threadId}/runs/{runId}/tool-results (check actual API path)
    ...
    - Mock client.stream and client.fetch as needed.
