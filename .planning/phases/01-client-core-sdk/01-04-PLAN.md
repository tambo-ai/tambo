...
<task type="auto">
  ...
  <action>
    First, read the typescript-sdk streaming code and the react-sdk to understand event types. Check:
    - node_modules/@tambo-ai/typescript-sdk/src/streaming.ts for SDK streaming support
    - node_modules/@tambo-ai/typescript-sdk/src/resources/threads/runs.ts for run methods
    - react-sdk/src/v1/hooks/ for SSE event handling patterns
    - apps/api/src/threads/ for what events the API actually sends

    Use the typescript-sdk's streaming capabilities where possible. Only build custom SSE parsing if the SDK doesn't expose the right async iteration interface.
...
<output>
After completion, create `.planning/phases/01-client-core-sdk/01-04-SUMMARY.md`
</output>
