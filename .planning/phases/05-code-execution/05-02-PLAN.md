...
must_haves:
  truths:
    - "CLI verifies files exist, are non-empty, and have basic valid syntax after execution"
    - "CLI displays actionable error messages with specific suggested fixes when operations fail"
    - "Execution orchestrator runs file writes, dependency install, and verification in correct order"
    - "On failure, all file changes are rolled back and user sees clear error with suggestions"
    - "On success, user sees summary of files created, modified, and dependencies installed"
  artifacts:
