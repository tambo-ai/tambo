...
1. **API key check**: Read API key from `.env.local` via `findTamboApiKey()` or `process.env.TAMBO_API_KEY`. Throw Error("No API key found. Please run `tambo init` first.") if missing. (Note: 06-02 ensures normal init runs first when --magic is used on uninitialized projects, so this function can assume init has already completed.)
