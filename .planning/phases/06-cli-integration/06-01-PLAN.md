...
1. **Uninitialized project check** (per locked decision: run full init first on uninitialized projects):
   - Check if project is initialized: look for `.tambo/config.json` or `.env.local` containing `TAMBO_API_KEY`
   - If project is NOT initialized (neither marker exists), log "Project not initialized. Running setup first..." and call `handleInit({ yes: options.yes, legacyPeerDeps: options.legacyPeerDeps, skipAgentDocs: options.skipAgentDocs })` (import from `./init.js`) — note: pass options WITHOUT magic flag to avoid recursion
   - After handleInit completes, proceed seamlessly to the magic pipeline below

2. **API key check**: Read API key from `.env.local` via `findTamboApiKey()` or `process.env.TAMBO_API_KEY`. If still missing after init ran above, throw Error("No API key found. Please run `tambo init` first.").

3. **Phase 2 — Analysis** with ora spinner:
...
5. **Phase 3 — Plan generation** with spinner:
...
7. **Phase 5 — Execution** with spinner:
...
- `isProjectInitialized()` — checks for `.tambo/config.json` or `.env.local` with TAMBO_API_KEY, returns boolean
