...
3. At the END of `handleInit()`, after the "Basic initialization complete!" message, add magic routing:

   Find the section around line 1126 that logs "Basic initialization complete!" and the Next steps. AFTER the entire normal init flow completes (auth, project creation, API key setup), add:

   ```typescript
   console.log(
     chalk.green("\n  ✓ ") + chalk.bold("Basic initialization complete!"),
   );
   ...
   // ... rest of existing "Next steps:" code
   ```

   This ensures the normal init flow (auth, project, API key) runs FIRST, then magic activates at the end. For already-initialized projects where the normal init flow would detect existing setup and skip most steps, it flows through quickly to the magic call.

4. After the "Basic initialization complete!" message (around line 1126), conditionally add the --magic suggestion. Only show it if magic setup hasn't already been done:
